...
class BlinkingLED(Module):
    def __init__(self, platform, led_pin,Pins):
        self.platform = platform
        self.led_pin = platform.request(led_pin,Pins)  
        self.counter = Signal(24)  # Counter 
        self.blink_count = Signal(4)  # 4-bit counter for counting blinks (up to 15)
        self.blink_active = Signal(reset=1)  # Signal to enable/disable blinking
        self.led_state = Signal()  # LED state signal
        
        self.sync += self.counter.eq(self.counter + 1)
        self.sync += self.led_pin.eq(self.counter[23])  # Toggle LED every counter cycle
        If(self.blink_count == 10,  # Stop after 10 blinks
        self.blink_active.eq(0),  # Disable blinking
        self.led_pin.eq(0)
        )
...
class IceSugarPlatform(LatticeECP5Platform):
    ...
    def __init__(self):  # Fix the constructor here
...
if __name__ == "__main__":
    platform = IceSugarPlatform()  # Instantiate the IceSugar platform
    led_blinker = BlinkingLED(platform, "user_led_n",0)  # Create Blinking LED instance
    platform.build(led_blinker)  # Build the design (generates files for the FPGA)
